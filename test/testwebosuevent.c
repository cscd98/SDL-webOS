/*
  ...
  Copyright (C) 2025-2026 Mariotaku <git@mariotaku.me>
...
typedef struct
{
    ...
    Uint64 netlink_time[PRESENCE_MAX_INDEX];
...
static Uint64 latency_total = 0;    /* how far netlink led the poll, summed */
...
static void Report(const char *fmt, ...)
{
    ...
    printf("[%3u.%03u] %s\n", (unsigned)(ms / 1000), (unsigned)(ms % 1000), msg);
    fflush(stdout);
}
 ...
 * yet. */
static void DrainMonitors(bool announce)
{
    int kind;
 ...
 * failed to report. */
static void PollPresence(bool announce)
{
    int kind;
    ...
    for (kind = 0; kind < NODE_KIND_COUNT; kind++) {
        ...
        for (index = 0; announce && index < PRESENCE_MAX_INDEX; index++) {
            ...
            if (cls->netlink_touched & bit) {
                Uint64 lead = SDL_GetTicks() - cls->netlink_time[index];
                latency_total += lead;
                latency_samples++;
                Report("poll     %-6s %s/%d (netlink was %ums ahead)",
                       (flags & bit) ? "add" : "remove", cls->label, index, (unsigned)lead);
            } else {
                poll_changes_missed++;
...
static int PrintVerdict(bool monitors_opened)
{
    printf("\n");
    printf("=========================================================\n");
    ...
    if (latency_samples > 0) {
        printf(" mean netlink lead over poll     : %ums over %d samples\n",
               (unsigned)(latency_total / (Uint64)latency_samples), latency_samples);
    }
...
static int RunSdlMode(Uint64 duration_ms)
{
    ...
    Report("SDL joystick subsystem up, %d joystick(s) already present", JoystickCount());
    Report("running for %us — plug and unplug a controller now", (unsigned)(duration_ms / 1000));
    ...
    while (keep_running && Elapsed() < duration_ms) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            case SDL_EVENT_JOYSTICK_ADDED:
                Report("SDL_EVENT_JOYSTICK_ADDED    instance id %u (%s)", (unsigned)event.jdevice.which,
                       SDL_GetJoystickNameForID(event.jdevice.which));
                break;
            case SDL_EVENT_JOYSTICK_REMOVED:
                Report("SDL_EVENT_JOYSTICK_REMOVED  instance id %u", (unsigned)event.jdevice.which);
                break;
            default:
                break;
            }
    ...
    Report("%d joystick(s) present at exit", JoystickCount());
...
{
    bool monitors_opened = true;
    bool sdl_mode = false;
    Uint64 duration_ms = 30000;
    Uint64 poll_interval_ms = 3000;
    Uint64 last_poll;
    int kind;
    int i;
    ...
    for (i = 1; i < argc; i++) {
        if (SDL_strcmp(argv[i], "--duration") == 0 && i + 1 < argc) {
            duration_ms = (Uint64)SDL_atoi(argv[++i]) * 1000;
        } else if (SDL_strcmp(argv[i], "--poll-interval") == 0 && i + 1 < argc) {
            poll_interval_ms = (Uint64)SDL_atoi(argv[++i]);
    ...
    while (keep_running && Elapsed() < duration_ms) {
        ...
        if (SDL_GetTicks() >= last_poll + poll_interval_ms) {
            PollPresence(true);
            last_poll = SDL_GetTicks();
        }
