...
// This is a PNG image file loading framework, originally written by
// Philippe Lavoie (2 November 1998) and vendored here from SDL_image.
...
{
    SDL_IOStream *src = (SDL_IOStream *)lib.png_get_io_ptr(ctx);
...
{
    (void)ctx;
    SDL_LogError(SDL_LOG_CATEGORY_SYSTEM, "libpng error: %s", msg);
}
...
SDL_Surface *IMG_LoadPNG_IO(SDL_IOStream *src)
{
    ...
    png_ptr = lib.png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, png_log_error, png_log_warn);
    if (png_ptr == NULL) {
    ...
    surface = SDL_CreateSurface((int)width, (int)height, format);
    if (surface == NULL) {
    ...
    if (ckey != -1) {
        if (color_type != PNG_COLOR_TYPE_PALETTE) {
            ...
            ckey = (int)SDL_MapSurfaceRGB(surface, (Uint8)transv->red, (Uint8)transv->green, (Uint8)transv->blue);
        }
        SDL_SetSurfaceColorKey(surface, true, (Uint32)ckey);
    }
     ...
     * In some cases it can't read PNGs created by some popular programs (ACDSEE),
    ...
    if (!palette && SDL_ISPIXELFORMAT_INDEXED(surface->format)) {
    ...
    if (palette) {
        ...
        if (ncolors > 0) {
            SDL_SetPaletteColors(palette, colors, 0, SDL_min(ncolors, palette->ncolors));
        }
    }
