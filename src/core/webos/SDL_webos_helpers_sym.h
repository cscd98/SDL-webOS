...
// Callback function called on an incoming message.
//
//   sh:    service handle (LSHandle)
//   reply: incoming message
//   ctx:   helper context
// Returns non-zero if the message is handled.
...
struct HContext
{
    ...
    void *userdata;
    void *unknown;
    // Whether the call is multiple (like a subscription), or one-shot.
    int multiple;
    // Whether the call is a public call or a private call.
    int pub;
