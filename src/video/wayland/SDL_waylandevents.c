                                  ...
                                  uint32_t time, wl_fixed_t sx_w, wl_fixed_t sy_w)
{
    ...
    // Filter out the call that was generated by pointer enter event (time is serial)
    if (window && window->waylandData->relative_mouse_mode && time != input->pointer_enter_serial) {
        if (!warping) {
            ...
            int dx = (int)SDL_floorf((float)wl_fixed_to_double(sx_w) - (float)cx);
            int dy = (int)SDL_floorf((float)wl_fixed_to_double(sy_w) - (float)cy);
            warping = true;
            // The position is always relative to the screen, not the window (even if the window is smaller and scaled)
