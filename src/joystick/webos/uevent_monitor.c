...
static bool ParseUevent(char *buf, size_t len, const char **subsystem, const char **devname,
                        SDL_webOSUeventAction *action);

static const char *TrailingName(const char *path);

static bool NodeNameMatches(const SDL_webOSUeventMonitor *monitor, const char *name);

static bool IsKnown(const SDL_webOSUeventMonitor *monitor, const char *name);
...
static bool QueueEvent(SDL_webOSUeventMonitor *monitor, SDL_webOSUeventAction action, const char *name);
...
bool SDL_webOSUeventMonitorPoll(SDL_webOSUeventMonitor *monitor, SDL_webOSUevent *event)
{
 ...
 *
 * Returns false for anything we can't turn into an add/remove, which
