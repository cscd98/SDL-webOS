...
{
    // There is no xdg-open here; the URL is handed to the system browser app.
    ...
    bool result = SDL_webOSLunaServiceCallSync("luna://com.webos.applicationManager/launch", payload, 1, NULL);

    PBNJSON_j_release(&payload_obj);

    if (!result) {
        return SDL_SetError("Couldn't open URL: %s", url);
    }
    return true;
}
