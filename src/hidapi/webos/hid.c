...
/* This is a fork of hidapi's linux/hid.c for LG webOS.
 *
 * The app jail has no udev running and no /sys/class/hidraw, so
 * udev_enumerate_scan_devices() comes back empty. hid_enumerate() below walks
 * /dev directly instead and turns each hidraw node into a udev_device with
 * udev_device_new_from_devnum(); everything after that is unchanged from the
 * Linux backend. It also honours SDL_HINT_WEBOS_HIDAPI_IGNORE_BLUETOOTH_DEVICES,
 * because the TV's own remotes and accessories pair over Bluetooth and appear
 * here alongside real controllers.
 *
 * Keep this file in sync with linux/hid.c when that is updated from upstream. */
...
/* False for a Bluetooth VID/PID listed in
   SDL_HINT_WEBOS_HIDAPI_IGNORE_BLUETOOTH_DEVICES, whose format matches
   SDL_HINT_HIDAPI_IGNORE_DEVICES: a list of "0xVVVV/0xPPPP" pairs, where a
   product of 0x0000 matches every product from that vendor. */
static int webos_bluetooth_device_allowed(unsigned short vendor_id, unsigned short product_id)
{
	const char *hint = SDL_GetHint(SDL_HINT_WEBOS_HIDAPI_IGNORE_BLUETOOTH_DEVICES);
...
static struct hid_device_info * create_device_info_for_device(struct udev_device *raw_dev)
{
	...
	switch (bus_type) {
		case BUS_BLUETOOTH:
			/* The TV pairs its own remotes and accessories over Bluetooth,
			   and they show up here as ordinary hidraw nodes. */
			if (!webos_bluetooth_device_allowed(dev_vid, dev_pid))
				goto end;
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	/* There is no udev daemon in the app jail, so the 'hidraw' subsystem
	   enumeration comes back empty. Walk /dev instead and build each device
	   from its device number. */
	dev_dir_fd = open("/dev", O_RDONLY | O_DIRECTORY);
	if (dev_dir_fd < 0) {
		register_global_error("Couldn't open /dev");
		udev_unref(udev);
		return NULL;
	}
	...
	if (dev_count < 0) {
		register_global_error("Couldn't scan /dev for hidraw devices");
		close(dev_dir_fd);
		udev_unref(udev);
		return NULL;
	}
	...
	for (dev_idx = 0; dev_idx < dev_count; dev_idx++) {
		...
		if (vendor_id != 0 || product_id != 0) {
			sysfs_path = udev_device_get_syspath(raw_dev);
			if (!sysfs_path ||
			    !parse_hid_vid_pid_from_sysfs(sysfs_path, &bus_type, &dev_vid, &dev_pid) ||
			    (vendor_id != 0 && vendor_id != dev_vid) ||
		...
		if (tmp) {
			...
			/* udev_device_get_devnode() has nothing to report inside the
			   jail, so name the node after the directory entry we found. */
			for (dev_info = tmp; dev_info; dev_info = dev_info->next) {
				free(dev_info->path);
				dev_info->path = (char *)malloc(sizeof("/dev/") + NAME_MAX);
				if (dev_info->path)
					snprintf(dev_info->path, sizeof("/dev/") + NAME_MAX, "/dev/%s", dev_list[dev_idx]->d_name);
			}
	...
	/* Free the device list and udev object. */
